					...
					mySqlConnection.query(
						...
						(error, result) => {
							if (error) {
								return res
									.status(400)
									.json({ error: "Post cannot be modified" });
							}
							return res.status(200).json(result);
						}
					);
...
exports.getAllPost = (req, res, next) => {
	mySqlConnection.query(
		"SELECT post.id, content, image, title, user_id, dateCreate, isAdmin, username  FROM post INNER JOIN user ON user.id = post.user_id ORDER BY dateCreate DESC",
		(error, result) => {
			if (error) {
...
exports.getOnePost = (req, res, next) => {
	mySqlConnection.query(
		"SELECT post.id, content, image, title, user_id, dateCreate, isAdmin, username FROM post INNER JOIN user ON user.id = post.user_id WHERE post.id=? ",
		req.params.id,
		(error, result) => {
			if (error) {
